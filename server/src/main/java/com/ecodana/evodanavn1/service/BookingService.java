...
public class BookingService {
    ...
    public BigDecimal getThisMonthRevenue() {
        ...
        LocalDateTime endOfMonth = LocalDateTime.now().withDayOfMonth(LocalDate.now().lengthOfMonth()).withHour(23)
                .withMinute(59).withSecond(59);
        List<Booking> monthBookings = bookingRepository.findByStatusAndDateRange(Booking.BookingStatus.Completed,
    ...
    public Map<String, Object> getBookingStatistics() {
        ...
        stats.put("cancelledBookings",
                allBookings.stream().filter(b -> b.getStatus() == Booking.BookingStatus.Cancelled).count());
        return stats;
    }
    ...
    public Map<String, Object> getBookingAnalytics() {
        ...
        try {
            ...
            analytics.put("dailyRevenue", bookingRepository.findDailyRevenue(sevenDaysAgo));
        } catch (Exception e) {
            analytics.put("dailyBookings", List.of());
            analytics.put("dailyRevenue", List.of());
        }
        try {
        ...
        try {
            ...
            analytics.put("yearlyRevenue", bookingRepository.findYearlyRevenue(fiveYearsAgo));
        } catch (Exception e) {
            analytics.put("yearlyRevenue", List.of());
        }
        try {
        ...
        return bookingRepository.findById(bookingId)
                .map(booking -> {
                    ...
                    if ("FULL".equals(paymentOption)) {
                        ...
                        BigDecimal deposit = booking.getTotalAmount().multiply(new BigDecimal("0.2")).setScale(2,
     ...
     */
    @Transactional
    public Booking completeBooking(String bookingId, User completer, String notes, List<String> imageUrls,
            boolean setMaintenance) throws Exception {
        ...
        log.setNote(notes);
        // Lưu ý: Chúng ta chưa yêu cầu owner nhập Odometer khi trả xe, nên tạm thời
        // không set
        ...
        // 7. Recalculate RemainingAmount and DepositAmountRequired based on all
        // completed payments
        ...
        // If the booking is fully paid (remaining amount is zero or less due to
        // rounding/overpayment)
        if (remainingAmount.compareTo(BigDecimal.ZERO) <= 0) {
            // If 100% of the total amount has been paid, set DepositAmountRequired to
            // totalAmount
            // and ensure RemainingAmount is zero.
            booking.setDepositAmountRequired(booking.getTotalAmount());
            booking.setRemainingAmount(BigDecimal.ZERO);
        } else {
            // If there's still a remaining amount after completion, it indicates an
            // unhandled payment
            // or a partial payment scenario. In this case, DepositAmountRequired should
            // retain its
            // original value (e.g., the initial 20% deposit), and RemainingAmount reflects
            // the unpaid balance.
            logger.warn(
                    "Booking {} completed with remaining amount: {}. This indicates an unhandled payment or partial payment.",
                    bookingId, remainingAmount);
            // DepositAmountRequired is not changed here, it keeps its value set during
            // approval.
        ...
        return bookingRepository.findById(bookingId)
                .map(booking -> {
                    ...
                    if (booking.getStatus() == Booking.BookingStatus.Pending
                        ...
                        // Nếu đã Confirmed (đã trả tiền), phải dùng logic
                        // cancelCar/processCancellationAndRefund
                        throw new IllegalStateException(
                                "Không thể hủy đơn đã thanh toán. Vui lòng sử dụng chức năng Hủy Xe.");
                    }
                })
                .orElse(null);
    }
    ...
    private void updateVehicleStatusOnBookingCompletionOrCancellation(Vehicle vehicle) {
        ...
        // Check if there are any other 'Approved' or 'Ongoing' bookings for this
        // vehicle
    ...
    public void autoRejectExpiredBookings() {
        ...
        for (Booking booking : expiredBookings) {
            try {
                ...
                logger.info("Đã tự động hủy booking: {} (Khách: {}, Xe: {})", bookingCode, customerId,
    ...
    /**
     * Tác vụ định kỳ: Tự động hủy các booking 'AwaitingDeposit' (chờ Khách) đã quá
     * hạn.
    ...
    public void autoCancelExpiredPayments() {
        ...
        if (expiredBookings.isEmpty()) {
            logger.info("Không có booking (AwaitingDeposit) nào quá hạn (Timeout = {} phút).",
                    customerPaymentTimeoutMinutes);
            return;
        }

        logger.warn("Tìm thấy {} booking (AwaitingDeposit) quá hạn thanh toán. Bắt đầu xử lý...",
        ...
        for (Booking booking : expiredBookings) {
            try {
                ...
                if (vehicle.getOwnerId() != null) {
                    notificationService.createNotification(
                            vehicle.getOwnerId(),
                            String.format("Đơn hàng #%s đã bị hủy do khách không thanh toán.",
                ...
                logger.error("Lỗi khi tự động HỦY (Cancelled) booking {}: {}", booking.getBookingId(), e.getMessage(),
                        e);
            }
        }
        bookingRepository.saveAll(expiredBookings); // Save all changes at once
    ...
    @Transactional
    public Map<String, Object> processCancellationAndRefund(String bookingId, String reason, User canceller,
            String bankAccountId, BigDecimal customRefundAmount) {
        ...
        if (booking.getStatus() != Booking.BookingStatus.Confirmed &&
            ...
            throw new IllegalStateException(
                    "Trạng thái đơn hàng không hợp lệ để hủy và hoàn tiền (" + booking.getStatus() + ").");
        }
        ...
        if (paymentTime == null) {
            ...
            throw new IllegalStateException(
                    "Không thể hủy vì không tìm thấy thời điểm thanh toán. Vui lòng liên hệ CSKH.");
        }
        ...
        logger.debug("[REFUND-CALC] Booking={}, Payments={}, Refundable={}",
                bookingId, payments.size(), refundablePayments.size());

        // Calculate hours since payment
        long hoursSincePayment = Duration.between(paymentTime, now).toHours();
        logger.debug("[REFUND-CALC] Hours since payment: {}", hoursSincePayment);
        ...
        for (Payment payment : refundablePayments) {
            ...
            if (hoursSincePayment < 2) {
                ...
                BigDecimal penalty = payment.getAmount().multiply(new BigDecimal("0.30")).setScale(2,
                        RoundingMode.HALF_UP);
                paymentRefundAmount = payment.getAmount().subtract(penalty);
                paymentRefundMessage = payment.getPaymentType() + ": Hoàn 70% (" + paymentRefundAmount
                        + " ₫, phí hủy 30%: " + penalty + " ₫)";
            }
            ...
            logger.debug("[REFUND-CALC] Payment type={}, Refund={}", payment.getPaymentType(), paymentRefundAmount);
        }
        ...
        logger.debug("[REFUND-CALC] Total refund amount={}, Message={}", totalRefundAmount, refundMessage);
        ...
        try {
            RefundRequest refundRequest = refundRequestService.createRefundRequest(booking, canceller, reason,
                    bankAccountId, totalRefundAmount);
            logger.debug("[REFUND-CALC] RefundRequest created: {}", refundRequest.getRefundRequestId());
            result.put("refundRequestId", refundRequest.getRefundRequestId());
        } catch (Exception e) {
            logger.error("[REFUND-CALC] Error creating RefundRequest: {}", e.getMessage(), e);
            // Vẫn tiếp tục, RefundRequest có thể được tạo sau từ sync endpoint
        }

        result.put("success", true);
        result.put("message", "Đã gửi yêu cầu hủy xe đến admin. " + refundMessage + " Số tiền hoàn dự kiến: "
                + totalRefundAmount.setScale(0, RoundingMode.HALF_UP) + " ₫");
        return result;
    }
     ...
     */
    @Transactional
    public Booking handoverVehicle(String bookingId, User owner, List<String> imageUrls, int odometer, String notes)
            throws Exception {
     ...
     * Chỉ tính các booking đã 'Completed' (Hoàn thành) hoặc 'Confirmed' (Đã thanh
     * toán cọc).
     * 
     * @param ownerId ID của chủ xe
     * @return Map chứa revenueToday, revenueThisMonth, revenueThisYear,
     *         totalRevenueAllTime
     */
    /**
     * Lấy phân tích doanh thu chi tiết cho một chủ xe cụ thể.
     ...
     * 
     * @param ownerId ID của chủ xe
     * @return Map chứa revenue... (tính theo TotalAmount) và totalPayoutAllTime
     *         (tính theo OwnerPayout)
     */
    ...
    public Map<String, Object> getOwnerRevenueChartData(String ownerId) {
        ...
        chartData.put("dailyLabels",
                dailyResults.stream().map(r -> r.get("period").toString()).collect(Collectors.toList()));
        chartData.put("dailyData",
                dailyResults.stream().map(r -> (BigDecimal) r.get("revenue")).collect(Collectors.toList()));
        ...
        List<Map<String, Object>> monthlyResults = bookingRepository.findMonthlyRevenueForOwner(ownerId,
                twelveMonthsAgo);
        chartData.put("monthlyLabels",
                monthlyResults.stream().map(r -> r.get("period").toString()).collect(Collectors.toList()));
        chartData.put("monthlyData",
                monthlyResults.stream().map(r -> (BigDecimal) r.get("revenue")).collect(Collectors.toList()));
        ...
        chartData.put("yearlyLabels",
                yearlyResults.stream().map(r -> r.get("period").toString()).collect(Collectors.toList()));
        chartData.put("yearlyData",
                yearlyResults.stream().map(r -> (BigDecimal) r.get("revenue")).collect(Collectors.toList()));
    ...
    public void processNoShowBookings(int gracePeriodInMinutes) {
        ...
        List<Booking> lateBookings = bookingRepository
                .findByStatusAndPickupDateTimeBefore(Booking.BookingStatus.Confirmed, threshold);
        ...
        for (Booking booking : lateBookings) {
            try {
                ...
                booking.setCancelReason(
                        "Tự động hủy do khách không đến nhận xe sau " + gracePeriodInMinutes + " phút.");
                ...
                notificationService.createNotification(booking.getUser().getId(),
                        "Đơn hàng #" + booking.getBookingCode() + " đã bị hủy do bạn không đến nhận xe.",
                        booking.getBookingId(), "BOOKING_NOSHOW");
                if (booking.getVehicle().getOwnerId() != null) {
                    notificationService.createNotification(booking.getVehicle().getOwnerId(),
                            "Đơn hàng #" + booking.getBookingCode() + " đã được chuyển sang trạng thái 'Không đến'.",
                            booking.getBookingId(), "BOOKING_NOSHOW");
                }
